...
@section('content')
    <form action="@if (Request::segment(3) == 'create-surat') {{ url('surat') }}@else{{ url('surat', $surat->id) }} @endif"
        ...
        <p style="text-align: justify">
            ...
            Nomor :
            <b><input type="text" name="no_persil" value="{{ $surat->no_persil ?? '' }}" placeholder="No Persil"
                    style="width:500px"></b> tanggal
            <b><input type="date" name="tgl_ipt" value="{{ $surat->tgl_ipt ?? '' }}" placeholder="Tanggal IPT"></b> yang
            terletak
            di <b><input type="text" name="alamat_persil" value="{{ $surat->alamat_persil ?? '' }}"
            ...
            {{ $data->alamat_ipt ?? '' }} mendasarkan pada :
        </p>
