                    ...
                    <form action="{{ url(Request::segment(1) . '/upload-bap') }}" method="POST"
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control" id="no_ipt" name="no_ipt" type="text"
                                value="{{ $arsip->no_persil ?? old('no_ipt') }}">
                        </div>
                        <div class="col-md-6 mb-3">
...
<div class="modal fade" id="verifikasi-ketua-petugas-modal" data-bs-backdrop="static" data-bs-keyboard="false"
    tabindex="-1" aria-labelledby="bap-modal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <div class="modal-toggle-wrapper">
                    <h4 class="text-center pb-2">Verifikasi Ketua {{ $data->no_permohonan }}?</h4>
                    <form action="{{ url(Request::segment(1) . '/verifikasi-surat') }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id_permohonan" value="{{ $data->id }}">
                        ...
                        <hr>
                        <div id="petugas-survey">
                            <div class="col-md-12 text-center mb-3">
                                <h5>Pilih Petugas Survey</h5>
                            </div>
                            <div class="form-check-size rtl-input mb-3">
                                @foreach ($petugasSurvey as $key => $item)
                                    <div class="form-check form-check-inline col-md-12">
                                        <input class="form-check-input me-2" id="inlineRadio{{ $key }}"
                                            type="radio" name="petugas_survey" value="{{ $item->id_user }}"
                                            checked="" required>
                                        <label class="form-check-label"
                                            for="inlineRadio{{ $key }}">{{ $item->nm_user }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="form-check-size rtl-input mb-3" id="status-verifikasi-ketua">
                            @foreach ($status as $key => $item)
                                <div class="form-check form-check-inline col-md-12">
                                    <input class="form-check-input me-2 pilihan-status" id="inlineRadio1"
                                        type="radio" name="action" value="{{ $key }}" checked=""
                                        required>
                                    <label class="form-check-label" for="inlineRadio1">{{ $item }}</label>
                                </div>
                            @endforeach
...
{{-- <div class="modal fade" id="kolektif-modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    aria-labelledby="bap-modal" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-body">
                <div class="modal-toggle-wrapper">
                    <h4 class="text-center pb-2">List Permohonan yang belum dibuatkan Surat</h4>
                    <form action="{{ url('surat' . '/save-list') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <table class="table">
                                <th>No</th>
                                ...
                                @foreach ($kolektif as $key => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <input type="checkbox" name="id_permohonan[]" id="id_permohonan"
                                                value="{{ $item->id }}"
                                                {{ in_array($item->id, $listKolektif) ? 'checked' : '' }}>
                                        </td>
                                    </tr>
                                @endforeach
                            </table>
